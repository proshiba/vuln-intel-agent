...
1. Claude routine（毎日 21:00 UTC、日次収集の main マージ後）がこの手順書に従って調査し、結果を `bot/threat-intel-daily` へ push する。
2. `.github/workflows/threat-intel-pr.yml` が検証（変更範囲が `vulndb/iocs/` と `vulndb/exports/` のみか、エクスポートが台帳と一致するか、ユニットテスト）を実行し、通れば PR を作成する。
3. **マージは人間が行う。** 各指標の出典 URL を開いて値が実際に載っているかを確認してからマージする（日次収集の auto-merge と違い、調査は言語モデルによる読み取りのため当面は目視を挟む）。
4. マージすると Pages が再デプロイされ、`malware` / `actor` / `ioc.*` エンティティがポータル索引に載る。
