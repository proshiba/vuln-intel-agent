...
現在有効な80ソースのCollector内訳は次のとおりです。
...
設定全体は160件、有効な収集源は80件です。tier内訳はdaily 74件、edge 6件です。
...
この30件も公式GitHub Repository Security Advisories APIを使用し、有効ソースを合計80件に
拡張しました。
...
2. `summarize`: 個別AI要約と、対象変更がある場合に必須の日次セクションAIサマリ、
   日次レポートを生成
3. `publish`: 生成ツリーを検証し、個別JSON、索引、状態、隔離データ、レポート、実行情報を
   すべてbot branchへcommitしてPRを作成
...
AIの任意依存は導入済みです。browser、PDFの任意依存は今回導入していません。

## 18. テストと現在の品質状態

今回の最終検証結果は次のとおりです。

- 設定検証: `160 sources, 80 enabled`
- Ruff lint: 成功
...
意図した認証案内付きで隔離されました。全80ソースの定期実行には専用tokenを使用します。

2026-07-18の認証済みdaily実行では全80ソースが成功し、隔離0件、アドバイザリ827件を
...
GitHub APIを利用する有効ソースは64件で、未認証の時間当たり上限を超えます。Collectorは
